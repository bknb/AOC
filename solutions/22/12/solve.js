...
function solve1(input) {
  log(`Start: ${S}, End: ${E}`);
  const minMap = Array.from(Array(input.length))
    .map(()=>Array.from(Array(input[0].length)).fill(Number.MAX_VALUE));
  const pos = [[[...S,heu(S[0],S[1],input[S[0]][S[1]]),'S',0],[]]];
  let min;
  let steps = 0;
  while(pos.length) {
    steps++;
    ...
    if (s) {
      ...
      for(let i=4;i-->0;) {
        ...
        if (nx>=0 && nx<input.length
          ...
          && !(min && path.length+minSteps(nx,ny) >= min.length))
  ...
  function heu(x,y) {
    return Math.abs(input[E[0]][E[1]]-input[x][y])
      + Math.abs(E[0]-x) + Math.abs(E[1]-y);
  }
}
