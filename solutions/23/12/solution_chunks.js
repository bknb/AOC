...
function patternSolutions([pattern, nums]) {
  if (!isValid([pattern,nums])) return 0;
  ...
  do {
    ...
    if(minNumsFor(pattern)>sum(...nums))
      return 0;
...
function chunkSolutions(chunks,nums) {
  ...
  return sum(...nums.map((n,i) => {
    ...
    const solution = patternSolutions([testChunk,testNums]);
    if (!solution) return 0;
    return solution * chunkSolutions(chunks.slice(1),nums.slice(i));
  }));
}

function validChunks(chunks,nums) {
  const allNums = sum(...nums);
  return sum(...chunks.map(c=>c.length)
             +chunks.length-1>=minLengthFor(nums))
    && sum(...chunks.map(c=>minNumsFor(c))<=allNums)
}

function isValid([p,n]) {
  const f = n[0], l = n[n.length-1];
  const check = `^\\?{0,${f}}#{${f+1},}|#{${l+1},}\\?{0,${l}}$`;
  return minNumsFor(p)<=sum(...n)
    && minLengthFor(n)<=p.length
    && !new RegExp(check).test(p);
}
...
function bruteForce([pattern,nums]) {
  ...
  const total = sum(...nums)-minNumsFor(pattern);
