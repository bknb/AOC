const {sum,create2DimArray,printMap,fillWithinBounds} = require('../../../santasLittleHelper.js');
...
function solve2(inp,l) {
  ...
  console.log(gcd(...input.map(([,n])=>n)));
  //return solve1(input,l);
}

function init(data,log) {
  return lineWise().match().map(([a,b,c])=>[letterDir[a],+b,c])(linify(data));
}
